...
class AllRoles extends Component {
    ...
    async fetchData(state, instance = null) {
        ...
        await this.props.makeRequest(request.Roles.all, query, {message: MESSAGES.LOGGING}).then(
            (responseData) => {
    ...
    async handleQuickEdit() {
        ...
        await this.props.makeRequest(request.Roles.update, data, {message: MESSAGES.LOGGING}).then(
            (responseData) => {
    ...
    async handleDelete() {
        ...
        const data = {
            role_ids: this.state.selectedIds
        };

        await this.props.makeRequest(request.Roles.deleteMultiple, data, {message: MESSAGES.LOGGING}).then(
            (responseData) => {
                this.setState({isLoading: false});
                this.fetchData(this.state.reactTableState);
            },
